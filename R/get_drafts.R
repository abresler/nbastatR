...
get_organization_type_ids <- function() {
  data <-
    data_frame(
      organization.type = c("", "College/University", "High School", "Other Team/Club"),
      id.organization.type = c(NA, "COL", "HS", "PRO")
    )
  return(data)
}
...
get_year_draft_data <-
  function(draft_year = 2015,
    ...
    oids_df <-
      get_organization_type_ids()

    draft_data %<>%
      left_join(oids_df) %>%
      dplyr::select(-organization.type) %>%
      mutate(
        is.from.high_school = ifelse(id.organization.type == 'HS', T, F),
        is.from.college = ifelse(id.organization.type == 'COL', T, F),
        is.from.pros = ifelse(id.organization.type == 'PRO', T, F)
      )

    pros_df <-
      draft_data %>%
      dplyr::filter(is.from.pros == T)
    ...
    pros_df %<>%
      separate(
        col = organization.from,
        into = c('pro.team', 'country.league'),
        remove = F,
        sep = '\\('
      ) %>%
      mutate(
        pro.team = pro.team %>% str_trim,
        country.league = country.league %>% str_replace_all('\\)', '') %>% str_trim
      )
    ...
    if (return_message == T) {
      "You got the " %>%
        paste0(draft_year, " NBA draft data") %>%
        message()
    }
...
get_all_draft_data <-
  function(draft_years = 1947:2015,
           return_message = T) {
    years <-
      draft_years
    rm <-
    ...
    all_drafts <-
      ...
      purrr::map(function(x)
        get_year_draft_data(draft_year = x, return_message = rm)) %>%
      compact %>%
      bind_rows
