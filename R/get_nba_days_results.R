    ...
    if (return_message == T) {
      if (team_leader_data %>% nrow > 0) {
        ...
        fun_fact <-
          '.\nDid you know that ' %>%
          paste0(
            fun_data$name.player.pts_leader,
            ' of the ',
            fun_data$name.team,
            '\nscored ',
            fun_data$pts,
...
get_days_games <-
  function(is.today = F,
    ...
    if(!get_nba_days_scores(date = date) %>%
       .$games %>% is.null){
      games.today <-
    ...
    if (include_back_to_back == T) {
      ...
      todays_teams %<>%
        mutate(
          played.yesterday = todays_teams$slug.team %in% played_yesterday$slug.team,
      ...
      home_teams <-
        ...
        dplyr::rename(
          slug.home_team = slug.team,
          is.home_team.played_yesterday = played.yesterday,
      ...
      away_teams <-
        ...
        dplyr::rename(
          slug.away_team = slug.team,
          is.away_team.played_yesterday = played.yesterday,
      ...
      t <-
        ...
        left_join(matchups) %>%
        mutate(
          both.back_to_back = ifelse(
            is.away_team.back_to_back == T &
              is.home_team.back_to_back == T,
            T,
            F
          )
        ) %>%
